...
size_type inversions_is2a(vector<int> a) {
    size_type count = 0;
    for(auto i=next(begin(a)); i!=end(a); ++i) {
        auto x = *i;
        decltype(i) j;
        for(j=i; j!=begin(a) && x<*prev(j); --j)
            *j = *prev(j);
        *j = x;
        count += distance(j, i);
    }
...
#define MS2_IS2a_CUTOFF 42 
size_type ms2_is2a_helper(vector<int>::iterator first, vector<int>::iterator last, vector<int>::iterator bfirst) {
    auto n = distance(first, last);
    size_type count = 0;

    if(n < MS2_IS2a_CUTOFF) {
        for(auto i=next(first); i!=last; ++i) {
            auto x = *i;
            decltype(i) j;
            for(j=i; j!=first && x<*prev(j); --j)
                *j = *prev(j);
            *j = x;
            count += distance(j, i);
        }
    ...
    auto count1 = ms2_is2a_helper(first, mid, bfirst);
    auto count2 = ms2_is2a_helper(mid, last, bmid);
...
size_type inversions_ms2_is2a(vector<int> a) {
    vector<int> b(a.size());
    return ms2_is2a_helper(begin(a), end(a), begin(b));
}

#define MS3_IS2_CUTOFF 17
size_type inversions_ms3_is2(vector<int> a) {
    auto n = a.size();
    size_type count = 0;

    // is2 algorithm
    for(auto first=begin(a); first<end(a); first+=MS3_IS2_CUTOFF) {
        auto last = min(first+MS3_IS2_CUTOFF,end(a));
        for(auto i=next(first); i!=last; ++i) {
            auto j = upper_bound(first, i, *i);
            rotate(j, i, next(i));
            count += distance(j, i);
    ...
    // ms3 algorithm
    vector<int> b(n);
    for(size_type width = MS3_IS2_CUTOFF; width < n; width *= 2) {
        for(size_type i=0; i<n; i+=2*width) {
            auto m = min(width, n-i);
            auto l = min(m+width, n-i);
            auto first = begin(a)+i;
